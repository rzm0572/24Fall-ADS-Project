...
参数：
x：要插入的元素
返回值：成功返回 true，检查到节点存在返回 false，
应使用 decreaseKey 来更新节点的值。
*/
template <class T>
...
参数：
x：要更新的节点
返回值：成功返回 true，若节点不存在或新值不小于当前值返回 false。
*/
template <class T>
bool BinaryHeap<T>::decreaseKey(Pair<T> x) {
    int pos = Position[x.key];                  // 获取节点在堆中的位置
    if (!checkExist(x.key) || Heap[pos] < x) {  // 节点不存在或新值不小于当前值，则不更新，返回 false
        return false;
    }
    Heap[pos] = x;                                  // 更新节点的值
    while (pos > 1 && Heap[pos] < Heap[pos / 2]) {  // 向上调整，直到父节点小于自己
